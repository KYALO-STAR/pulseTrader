...
function initOverUnder() {
  ...
  ouThresholdEl.addEventListener("change", () => { updateOverUnder(); updateMatchesDiffers(); });
}
...
function updateOverUnder() {
  ...
  if (ouSeqChipsEl) {
    ...
    for (let i = digitsQueue.length - n; i < digitsQueue.length; i++) {
      ...
      let cls = "equal"; let text = "=";
      if (v < thresholdNow) { cls = "under"; text = "U"; }
      else if (v > thresholdNow) { cls = "over"; text = "O"; }
