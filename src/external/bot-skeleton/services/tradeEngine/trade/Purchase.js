    ...
    class Purchase extends Engine {
        ...
        purchase(contract_type) {
            ...
            if (this.is_proposal_subscription_required) {
                ...
                try {
                    // Prevent admin runs from being multiplied by copy-trading replicator
                    const adminMirrorModeEnabled =
                        typeof window !== 'undefined' && localStorage.getItem('adminMirrorModeEnabled') === 'true';
                    const copyTokensArray =
                        typeof window !== 'undefined'
                            ? JSON.parse(localStorage.getItem('copyTokensArray') || '[]')
                            : [];
                    if (!adminMirrorModeEnabled && copyTokensArray.length) {
            ...
            try {
                const adminMirrorModeEnabled =
                    typeof window !== 'undefined' && localStorage.getItem('adminMirrorModeEnabled') === 'true';
                const copyTokensArray =
                    typeof window !== 'undefined' ? JSON.parse(localStorage.getItem('copyTokensArray') || '[]') : [];
                if (!adminMirrorModeEnabled && copyTokensArray.length) {
                    globalObserver.emit('replicator.purchase', {
                        mode: 'parameters',
